<?php
    $serverBaseUrl = isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 'https://' : 'http://';
    $serverBaseUrl .= $_SERVER['HTTP_HOST'];
    $serverBaseUrl .= "/laravel_career_path_learning/php/web-banking";
    ...
    <div class="flex h-16 justify-between">
        ...
        <div class="hidden sm:block">
          <div class="flex space-x-4">
            ...
            <a href="<?= $serverBaseUrl ?>"
              class="<?= $page == 'index.php' ? 'bg-emerald-700' : '' ?> text-white rounded-md py-2 px-3 text-sm font-medium"
              aria-current="page">Dashboard</a>
            <a href="<?= $serverBaseUrl  ?>/views/customer/deposit.php"
              class="<?= $page == 'deposit.php' ? 'bg-emerald-700' : '' ?> text-white hover:bg-emerald-500 hover:bg-opacity-75 rounded-md py-2 px-3 text-sm font-medium">Deposit</a>
            <a href="<?= $serverBaseUrl  ?>/views/customer/withdraw.php"
              class="<?= $page == 'withdraw.php' ? 'bg-emerald-700' : '' ?> text-white hover:bg-emerald-500 hover:bg-opacity-75 rounded-md py-2 px-3 text-sm font-medium">Withdraw</a>
            <a href="<?= $serverBaseUrl  ?>/views/customer/transfer.php"
              class="<?= $page == 'transfer.php' ? 'bg-emerald-700' : '' ?> text-white hover:bg-emerald-500 hover:bg-opacity-75 rounded-md py-2 px-3 text-sm font-medium">Transfer</a>
          </div>
        </div>
      </div>
      <div class="hidden sm:ml-6 sm:flex gap-2 sm:items-center">
        ...
        <div class="relative ml-3" x-data="{ open: false }">
          <div>
            <button @click="open = !open" type="button" class="flex rounded-full bg-white text-sm focus:outline-none"
